...
import {
    syncFPTopicAttendanceSheet,
    readFPTopicAttendanceSheet,
    readAllFPTopicAttendanceSheets,
    clearFPTopicAttendanceSheet,
    deleteFPSpreadsheetTab,
    buildFPTopicTabTitle,
    linkFPDocument,
...
export default function FPTopicAttendancePage() {
    ...
    const filteredSheets = !activeCourse || selectedCourseId === 'ALL'
        ...
        : sheets.filter((s) => {
            ...
            const courseSpreadsheetId = activeCourse.links?.topicAttendance?.spreadsheetId
    ...
    useEffect(() => {
        ...
        filteredSheets.forEach((s) => {
            const needsUpdate = (!s.cursoId && activeCourse.id) ||
                ...
                (!s.spreadsheetId && activeCourse.links?.topicAttendance?.spreadsheetId)
            if (needsUpdate) {
                updateSheet(s.id, {
                    cursoId: s.cursoId || activeCourse.id,
                    cursoNumero: s.cursoNumero || activeCourse.cursoNumero,
                    spreadsheetId: s.spreadsheetId || activeCourse.links?.topicAttendance?.spreadsheetId || '',
                })
            }
        })
    ...
    const handleAddNewSheet = () => {
        ...
        const existingMeses = filteredSheets.map((s) => (s.mesDe || s.googleSheetTitle || '').trim().toLowerCase())
        ...
        setSelectedId(newId)
        toast.success(`Nueva pestaña "${nextMonth.toUpperCase()}" para Curso Nº ${activeCourse.cursoNumero || '—'}`)
    }
    ...
    const handleSyncSingle = async (sheetToSync) => {
        ...
        try {
            ...
            toast.success(`"${sheetToSync.googleSheetTitle || sheetToSync.mesDe || 'Planilla'}" sincronizada con Google Sheets`)
    ...
    const handleConfirmDelete = async () => {
        ...
        const targetName = sheetToDelete.googleSheetTitle || sheetToDelete.mesDe || sheetToDelete.especialidad || 'Planilla'
        ...
        try {
            if (targetSheetId) {
                ...
                const enrichedSheets = allSheets.map((s) => ({
                    ...
                    cfpNumero: s.cfpNumero || activeCourse?.cfpNumero || '',
                    distrito: s.distrito || activeCourse?.distrito || '',
                    sedeDictado: s.sedeDictado || activeCourse?.lugarDictado || '',
    ...
    const handlePullFromSheets = async (targetSheetId = null) => {
        ...
        if (activeCourse && selectedCourseId !== 'ALL') {
            ...
            const confirmMsg = `Esto va a sincronizar las pestañas de Tema y Asistencia del Curso Nº ${activeCourse.cursoNumero || '—'} con Google Sheets. ¿Continuar?`
            if (!window.confirm(confirmMsg)) return
    ...
    return (
        <div className="space-y-6 animate-fade-in">
            ...
            <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                    <h1 className="text-2xl font-bold text-text-primary">Tema y Asistencia</h1>
                    <p className="text-sm text-text-secondary mt-1">Formación Profesional — Registro Diario de Clases y Asistencia del Instructor</p>
                </div>
                <div className="flex flex-wrap items-center gap-2">
                    <Button
                        ...
                        title="Descarga las planillas de tema y asistencia desde Google Sheets hacia este dispositivo"
                    >
            ...
            {selectedCourseId === 'ALL' && (
                <div className="space-y-4">
                    <h2 className="text-base font-semibold text-text-primary">Todas las Planillas de Tema y Asistencia</h2>
                            ...
                            <CardBody className="text-center py-12 space-y-4">
                                <p className="text-text-secondary">
                                    {courses.length === 0
                                        ...
                                        : `Todavía no hay planillas para el Curso Nº ${activeCourse?.cursoNumero || '—'}.`}
                                </p>
                                {activeCourse && (
                            ...
                            <div className="bg-bg-surface border border-border-light rounded-xl p-2 shadow-sm">
                                <div className="flex items-center justify-between gap-2 mb-2 px-2 pt-1">
                                    <span className="text-xs font-bold text-text-secondary uppercase tracking-wider flex items-center gap-1.5">
                                        📑 Pestañas de Tema y Asistencia (Meses)
                                    </span>
                                ...
                                <div className="flex items-center gap-2 overflow-x-auto pb-1 scrollbar-thin">
                                    {filteredSheets.map((s, idx) => {
                                        ...
                                        const tabTitle = s.googleSheetTitle || s.mesDe || `Pestaña ${idx + 1}`
                                        return (
                                            <button
                                                key={s.id}
                                            ...
                                            >
                                                <NotebookPen className={`w-4 h-4 ${isActive ? 'text-white' : 'text-text-muted'}`} />
                                                <span>{tabTitle}</span>
                            ...
                            <div className="flex flex-wrap items-center justify-between gap-3 px-4 py-3 bg-bg-surface border border-border-light rounded-xl shadow-sm">
                                <div>
                                    <h2 className="text-base font-bold text-text-primary flex items-center gap-2">
                                        <span>Planilla: {selected.googleSheetTitle || selected.mesDe || 'Sin mes'}</span>
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <button
                                        ...
                                        title="Eliminar esta pestaña mensual"
                                    >
                                        <Trash2 className="w-4 h-4" />
                                    </button>
                                </div>
                                            ...
                                            {selected.entries.length === 0 && (
                                                <tr>
                                                    <td colSpan={7} className="py-6 text-center text-text-muted">
                                                        No hay clases registradas todavía en esta pestaña. Hacé clic en "Agregar Clase" para registrar una.
                                                    </td>
                                                </tr>
                                            )}
                                        </tbody>
            ...
            {/* Modal de confirmación al eliminar planilla / pestaña */}
            <ConfirmModal
                isOpen={!!sheetToDelete}
                onClose={() => setSheetToDelete(null)}
                onConfirm={handleConfirmDelete}
                title="¿Desea borrar realmente esta pestaña?"
                description={`¿Está seguro de que desea eliminar la pestaña "${sheetToDelete?.googleSheetTitle || sheetToDelete?.mesDe || 'esta pestaña'}" (Curso Nº ${sheetToDelete?.cursoNumero || '—'})? Esta acción no se puede deshacer.`}
                confirmLabel="Sí, eliminar pestaña"
            />

            {/* Modal para elegir cuál planilla sincronizar si hay varias (Modo ALL) */}
            <Modal
